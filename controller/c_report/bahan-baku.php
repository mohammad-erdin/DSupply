...
if($XLS){	
	...
	header('Content-Disposition: attachment;filename="rekap-bahan.xlsx"');
	...
	while($qry && $isi = mysql_fetch_assoc($qry)){
		...
		if($x==1){
			...
			$excel->getActiveSheet()->mergeCells("A".$j.":I".$j)->setCellValue('A'.$j,$nmBahan);
		...
		$excel->getActiveSheet()
			->setCellValue('A'.$j, $x)
			->setCellValue('B'.$j, $isi["Nama_Cabang"])
	...
	while($qry && $isi = mysql_fetch_assoc($qry)){
		...
		if($x==1){
			$Data.="<tr><td class='BL BR BB' colspan='5'>&nbsp;</td></tr>
					<tr><td class='BL BR BB' colspan='5' style='background-color:#D2D2D2'>$nmBahan</td></tr>";		
		}
		$Data.="
			<tr>
				<td class='BL BB' align='center'>$x</td>
				<td class='BL BB'>".$isi["Nama_Cabang"]."</td>
	 ...
	 <body>
		<table cellpadding=2 cellspacing=0 style='width:21cm'>
			...
			<tr height='40px'>
				...
				<th class='BL BT BB' style='background-color:#eeeeee;'>Nama Cabang</th>
